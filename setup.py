...
import vstools  # pylint: disable=wrong-import-position
        ...
        if line.startswith('Summary: '):
          ...
          if python_package == 'python3':
            lines.extend([
                ...
                '%{python3_sitelib}/vstools*.egg-info/*',
                '',
                '%exclude %{_prefix}/share/doc/*',
                '%exclude %{python3_sitelib}/vstools/__pycache__/*',
                '%exclude %{_bindir}/*.py'])

          else:
            lines.extend([
                '%{python2_sitelib}/vstools/*.py',
                '%{python2_sitelib}/vstools*.egg-info/*',
                '',
                '%exclude %{_prefix}/share/doc/*',
                '%exclude %{python2_sitelib}/vstools/*.pyc',
                '%exclude %{python2_sitelib}/vstools/*.pyo',
                '%exclude %{_bindir}/*.py'])
          ...
          if python_package == 'python2':
            python_spec_file.extend([
                'Obsoletes: python-vstools < %{version}',
                'Provides: python-vstools = %{version}'])
